...
Descripción: Permitir la creación de un nuevo registro de persona en la base de datos.
Precondiciones:
La persona debe tener un nombre, apellido, ciudad, dirección, edad, correo electrónico y
género válidos.
...
this_proc:BEGIN
    ...
    -- verify if the gender exists
    SELECT COUNT(*) INTO genderExists
    FROM gender g
    WHERE g.id = gender_id;
    
    IF (genderExists = 0) THEN
		SET response = CONCAT("Ups!, parece que el genero con id ",gender_id," no existe. Por favor intente nuevamente.");
        LEAVE this_proc;
	END IF;
    
    -- if all validation false, insert the data into person's table
    INSERT INTO person (name, lastName, idcity, address, age, email, idgender)
    VALUES (name, lastName, cityId, address, age, email, gender_id);
    
    -- set successful response
    SET response = "Persona agregada exitosamente!";
    LEAVE this_proc;
    
END$$

CREATE PROCEDURE add_skill_person (
	IN register_date DATE,
    IN person_id INT,
    IN skill_id INT,
    OUT response VARCHAR(200)
)
this_proc:BEGIN
    ...
    IF (personExists = 0) THEN
		SET response = CONCAT("Ups!, parece persona con id ",person_id," no existe. Por favor intente nuevamente.");
        LEAVE this_proc;
	END IF;
    
    -- verify if the skill exists
    SELECT COUNT(*) INTO skillExists
    FROM skill s
    WHERE s.id = skill_id;
    
    IF (skillExists = 0) THEN
		SET response = CONCAT("Ups!, parece que la habilidad con id ",skill_id," no existe. Por favor intente nuevamente.");
        LEAVE this_proc;
	END IF;
    
    -- verify if the person has the skill
    SELECT COUNT(*) INTO personHasSkill
    FROM persons_skill ps
    WHERE (ps.idperson = person_id) AND (ps.idskill = skill_id);
    
    IF (personHasSkill > 0) THEN
		SET response = CONCAT("Ups!, esta persona ya tiene esta habilidad.");
        LEAVE this_proc;
	END IF;
    
    -- if all verifications are false insert the data
    INSERT INTO persons_skill (registration_date, idperson, idskill)
	VALUES (register_date, person_id, skill_id);
    
    -- set successful message
    SET response = CONCAT("Habilidad agregada exitosamente!");
END$$

CREATE PROCEDURE create_skill (
	IN skill_name VARCHAR(50),
    OUT response VARCHAR(200)
)
this_proc:BEGIN
	DECLARE skillExists INT;
    ...
    IF (skillExists > 0) THEN
		SET response = "Ups!, parece que esta habilidad actualmente existe. Trate nuevamente";
        LEAVE this_proc;
	END IF;
    
	-- if false 
    INSERT INTO skill (name) VALUES (skill_name);
    SET response = "Habilidad creada exitosamente!";
END$$

CREATE VIEW personsView AS
SELECT p.id, CONCAT(p.name," ",p.lastName) AS individuo, g.name AS genero, s.name AS habilidad
FROM persons_skill ps
JOIN person p
ON p.id = ps.idperson
JOIN gender g
...
CREATE PROCEDURE get_persons_by_skill (
	IN skill_name VARCHAR(50)
)
this_proc:BEGIN
    -- query
...
this_proc:BEGIN
    ...
    IF (personExists = 0) THEN 
		SET response = CONCAT("Ups! parece que la persona con id ",id," no existe. Porfavor intenta nuevamente.");
        LEAVE this_proc;
	END IF;
    ...
    SELECT COUNT(*) INTO genderExists
    FROM gender g
    WHERE g.id = newGenderId;
    
    IF (genderExists = 0) THEN
		SET response = CONCAT("Ups! parece que el genero con id ",newGenderId," no existe. Por favor intenta nuevamente.");
        LEAVE this_proc;
	END IF;
    ...
    SET p.name = newName,
        ...
        p.age = newAge,
        p.email = newEmail,
        p.idgender = newGenderId
	WHERE p.id = id;
    
    -- set response
    SET response = "Persona modificada exitosamente!";
END$$
...
this_proc:BEGIN
    ...
    IF (personExists = 0) THEN
		SET response = CONCAT("Ups! parece que la persona con id ",id," no existe. Por favor intenta nuevamente.");
        LEAVE this_proc;
	END IF;
    
    -- if all false
    DELETE FROM person p WHERE p.id = id;
    SET response = "Persona eliminada exitosamente!";
END$$
DELIMITER ;